...
public class Base58 {
    ...
    public static String encodeAddress(byte prefix, byte[] bytes) {
        // A stringified buffer is:
        // 1 byte prefix + data bytes + 4 bytes check code (a truncated hash)
        byte[] addressBytes = new byte[1 + bytes.length + 4];
        addressBytes[0] = prefix;
        System.arraycopy(bytes, 0, addressBytes, 1, bytes.length);
        byte[] checksum = ByteUtil.sha256sha256(Arrays.copyOfRange(addressBytes, 0, bytes.length + 1));
